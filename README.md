# MUMU 음악 탐험대

박, 가락의 높낮이, 프레이즈와 음색을 들으며 연주하는 초등학생용 감상 활동 리듬게임입니다.

## 실행

```powershell
npm install
npm run dev
```

기본 주소는 `http://127.0.0.1:5173`입니다.

## 빌드

```powershell
npm run build
```

## 조작

- 키보드: `A`, `S`, `D`, `J`, `K`
- 태블릿: 하단 5개 패드 터치
- 일시정지: `Escape`

## 설계 원칙

- 낮은음에서 높은음이 왼쪽에서 오른쪽으로 이동합니다.
- 실제 가락 데이터로 채보하며 무작위 레인 이동을 사용하지 않습니다.
- 실수해도 곡은 끝까지 이어집니다.
- 리듬 점수와 감상 이해를 분리해서 보여 줍니다.

## 악기 음원

- 반짝반짝 작은 별: 첼레스타 멜로디 + 피아노 반주
- 환희의 노래: 그랜드 피아노 멜로디 + 피아노 반주
- 달빛 숲의 왈츠: 플루트 멜로디 + 피아노 반주
- 터키 행진곡: 피아노 + 현악 + 트럼펫 + 팀파니
- 캉캉: 바이올린 + 플루트 + 피아노 + 타악기
- 윌리엄 텔 서곡: 트럼펫 + 바이올린 + 팀파니
- 헝가리 무곡 5번: 바이올린 + 피아노 + 현악 앙상블
- 피아노, 첼레스타, 플루트, 현악 앙상블, 글로켄슈필, 바이올린, 트럼펫, 팀파니의 필요한 음역만 포함합니다.
- 완성 반주는 곡의 마디·화음 데이터에서 생성되어 화면 노트와 같은 오디오 시계를 사용합니다.

빠른 클래식의 차트와 완성 반주는 같은 악보 타임라인에서 생성합니다.

```powershell
pip install numpy music21
python tools/render_classics.py
```

서드파티 음원 출처와 라이선스는 `THIRD_PARTY_ASSETS.md`에 정리되어 있습니다.
